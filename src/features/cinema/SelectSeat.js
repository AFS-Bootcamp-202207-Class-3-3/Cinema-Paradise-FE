...
function SelectSeat() {
  ...
  const toggleSeat = (e) => {
    if (
      e.target.getAttribute("src") === "/assets/img/seats0.gif" &&
      selectedIndex === -1
    ) {
  ...
  const submitSeat = () => {
    const seatStr = `${Math.floor(selectedIndex / 100) + 1}排${
      Math.floor(selectedIndex % 100) + 1
    }列`;
    ...
    <>
      ...
      {seatsArray.map((row, rowIndex) => (
        <Row justify="center" align="top" key={"row" + rowIndex}>
          {row.map((seatNumber, colIndex) =>
            chosenSeatsKeys.includes(seatNumber) ? (
              <Popover
                content="这个位子被选掉了(っ °Д °;)っ"
                key={"row" + rowIndex + "col" + colIndex + "pop"}
              >
                <Col
                  span={1}
                  className="block-height-60"
      ...
      <Row justify="center">
        <Col>
          <Text className="block-height-60">
            ...
            {selectedIndex === -1
              ? "未选择"
              : `${Math.floor(selectedIndex / 100) + 1}排${
                  Math.floor(selectedIndex % 100) + 1
                }列`}
          </Text>
        </Col>
      ...
      <Row justify="center">
        <Col className="block-height-60">
          <Link to="/purchase">
            <Button onClick={submitSeat} disabled={!hasSelected}>
              提交，选择支付方式
            </Button>
          </Link>
        </Col>
